...
public class FilesMonitorWithProvider : BackgroundService
{
    ...
    public FilesMonitorWithProvider(IOptionsMonitor<StorageConfiguration> optionsMonitor, ILogger<FilesMonitorWithProvider> logger)
    {
        this._configuration = optionsMonitor.CurrentValue;
        this._logger = logger;

		var directoryToMonitor = Directory.CreateDirectory(this._configuration.Root);

        _physicalFileProvider = new PhysicalFileProvider(directoryToMonitor.FullName);
    }
    ...
    void CheckDirectoryChanges(object state)
    {
        ...
        var currentDirectoryState = GetDirectoryState(_physicalFileProvider, this._configuration.Root);
